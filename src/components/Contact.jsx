export default function Contact() {
  ...
  return (
    <footer id="contact" className="bg-white px-4 py-8">
      <div className="mx-auto max-w-3xl rounded-[26px] bg-[#fafafa] px-6 py-8 shadow-none border border-[#f0f0f0]">
        <div className="text-center">
          <h2 className="text-4xl font-bold tracking-tight text-[#1d1d1d]">{profile.name}</h2>
          <p className="mt-2 text-2xl text-[#0f0f0f] opacity-80">{profile.title}</p>
        </div>
        ...
        <div className="mt-8 rounded-[24px] bg-[#f0f0f0] p-4 shadow-sm">
          ...
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#e7e7e7] text-lg text-[#3a7fe0]">✉</div>
            <div>
              <div className="text-xl font-medium text-[#2d2d2d]">Email</div>
              <div className="text-2xl font-normal text-[#0d0d0d]">{profile.email}</div>
            </div>
          </a>
